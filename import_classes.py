...
def timePlaceFromString(timeAndPlace):
    timePlace = timeAndPlace.split()
    if len(timePlace) == 4:
        time = timePlace[0:2]
        place = timePlace[3]
    if timePlace[0] == "*TO":
...
for cl in classes:
    ...
    if cltype == 'LectureSession':
        ...
        label = cl['label']
        time, place = timePlaceFromString(cl['timeAndPlace'])
